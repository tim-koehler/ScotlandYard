...
class GameInitializer() extends GameInitializerInterface{
  ...
  override def initialize(nPlayers: Int, stationsSource: String): GameModel = {
    ...
    var station1 = Station(1, StationType.Underground, false, Set(), Set(), Set())
    var station2 = Station(2, StationType.Taxi, false, Set(station1), Set(), Set())
    val station3 = Station(3, StationType.Taxi, false, Set(station1), Set(), Set())
    val station4 = Station(4, StationType.Bus, false, Set(station1), Set(), Set())
    var station5 = Station(5, StationType.Underground, false, Set(station1, station2), Set(), Set())
    station1 = station1.copy(neighbourTaxis = Set(station2, station3, station4, station5), neighbourBuses = Set(station5, station4), neighbourUndergrounds = Set(station5))
    station2 = station2.copy(neighbourTaxis = Set(station2, station3, station4, station5), neighbourBuses = Set(station5, station4), neighbourUndergrounds = Set(station5))
    station5 = station5.copy(neighbourTaxis = Set(station1, station2, station3, station4), neighbourBuses = Set(station1, station4), neighbourUndergrounds = Set(station1))
