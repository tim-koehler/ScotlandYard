...
class Tui(controller: ControllerInterface, tuiMap: TuiMapInterface) extends Reactor {
  ...
  val titleBanner: String = getBanner("./resources/tui_titleBanner.txt")
  var detectiveWinningBanner: String = getBanner("./resources/tui_detectiveWinBanner.txt")
  var mrXWinningBanner: String = getBanner("./resources/tui_mrXWinBanner.txt")
  ...
  def buildOutputStringForRunningGame(): String = {
    ...
    for(t <- controller.getMrX.history.reverse) {
  ...
  def getMrXStartingPositionString(): String = {
    "MrX is at Station: " + controller.getCurrentPlayer.station + "\n\n\n\n\n"
  }
  ...
  def buildOutputStringWin(): String = {
    if (controller.getWinningPlayer().name.equals("MrX")) {
      mrXWinningBanner + "\n\n" +
        ...
        " was at Station " + controller.getWinningPlayer().station + " !!!\n\n" +
        "Enter any key to exit..."
    } else {
      detectiveWinningBanner + "\n\n" +
        controller.getWinningPlayer().name + " has caught " + controller.getMrX.name +
        " at Station " + controller.getWinningPlayer().station + " !!!\n\n" +
        "Enter any key to exit..."
    }
  }
