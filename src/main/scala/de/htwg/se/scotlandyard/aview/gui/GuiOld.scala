...
object GuiOld extends JFXApp{
  ...
  stage = new PrimaryStage {
    ...
    scene = new Scene(rootPane, frameWidth, frameHeight) {
  ...
  scene2 = new Scene {
    ...
    var setNumberButton = new Button("Set Number") {
      onAction = (event: ActionEvent) => {
        ...
        playerList = new ListView[String](List(GameMaster.players(0).name, GameMaster.players(1).name))
